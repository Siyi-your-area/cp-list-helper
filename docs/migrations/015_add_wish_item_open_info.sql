-- Persist the existing product-level 开摊信息 field used by desktop and mobile editors.
...
begin
  ...
  update public.wish_items w set
    ...
    open_info = case when p_patch ? 'open_info' then nullif(p_patch->>'open_info', '') else w.open_info end,
...
revoke all on function public.save_wish_item_cas(text,uuid,bigint,jsonb) from public, anon;
