...
test("list 统计按五种状态形成总数、有料和无料", () => {
  ...
  assert.equal(summary.paid, 3);
  assert.equal(summary.free, 2);
  assert.equal(summary.total, 5);
});

test("预计花费统计全部制品，实际花费只统计已购买", () => {
  const summary = calculateListSummary([
    item("pending", "pending", 30, 2),
    item("purchased", "purchased", 45, 2),
    item("soldout", "soldout", 20, 1),
  ...
  assert.equal(summary.estimatedCost, 170);
  assert.equal(summary.actualCost, 90);
});
...
test("电脑和手机共用一套统计条，并支持悬停或点击展开明细", () => {
  ...
  assert.match(component, /activeKind === "paid"[\s\S]*?待购买[\s\S]*?已购买[\s\S]*?已售罄/);
